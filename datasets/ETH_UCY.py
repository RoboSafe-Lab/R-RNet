...
class ETHUCYDataset(data.Dataset):
    def __init__(self, cfg, split):
        ...
        hyperparams['minimum_history_length'] = self.cfg.MODEL.INPUT_LEN-1 if self.split == 'test' else 7   # test:7 train:1
        hyperparams['maximum_history_length'] = self.cfg.MODEL.INPUT_LEN-1  # 7
        ...
        if split=='train':
            ...
            augment = True if self.cfg.DATASET.AUGMENT else False   # True
        else:
            min_history_timesteps = 7
        self.dataset = NodeTypeDataset(train_env, 
                                        ...
                                        scene_freq_mult=hyperparams['scene_freq_mult_train'], #false
                                        node_freq_mult=hyperparams['node_freq_mult_train'], #false
                                        hyperparams=hyperparams, 
                                        augment=augment,                                    #true
                                        min_history_timesteps=min_history_timesteps,     # test:7 train:1
                                        min_future_timesteps=hyperparams['prediction_horizon'], #12
                                        return_robot=False)
    ...
    def __getitem__(self, index):
        first_history_index, x_t, y_t, x_st_t, y_st_t, neighbors_data, neighbors_data_st, neighbors_lower_upper, neighbors_future, \
            neighbors_edge_value, robot_traj_st_t, map_tuple, scene_name, timestep = self.dataset.__getitem__(index) # 1
        ret = {}
...
class NodeTypeDataset(data.Dataset):
    ...
    def __init__(self, env, node_type, state, pred_state, node_freq_mult,
        ...
        self.index = self.index_env(node_freq_mult, scene_freq_mult, **kwargs)  #4
        self.len = len(self.index)
        self.edge_types = [edge_type for edge_type in env.get_edge_types() if edge_type[0] is node_type]
    def index_env(self, node_freq_mult, scene_freq_mult, **kwargs): # 5          x,x,{1/7,12,x}
        index = list()
        
        for scene in self.env.scenes:
            present_node_dict = scene.present_nodes(np.arange(0, scene.timesteps), type=self.node_type, **kwargs)   #6
    ...
    def __getitem__(self, i):   # 2
        (scene, t, node) = self.index[i]    # 3
