...
def print_info(epoch, model, optimizer, loss_dict, logger, cfg):
    ...
    if cfg.MODEL.ATTENTION:
        info = "Epoch:{},\t lr:{:6},\t loss_goal:{:.4f},\t loss_attention:{:.4f},\t loss_traj:{:.4f},\t loss_probability:{:.4f},\t loss_kld:{:.4f},\t \
            kld_w:{:.4f},\t z_clip:{:.4f} ".format( 
            epoch, optimizer.param_groups[0]['lr'], loss_dict['loss_goal'], loss_dict['loss_attention'], loss_dict['loss_traj'], loss_dict['loss_probability'],
            loss_dict['loss_kld'], loss_dict['kld_weight'], loss_dict['z_logit_clip'])
    else:
        info = "Epoch:{},\t lr:{:6},\t loss_goal:{:.4f},\t loss_traj:{:.4f},\t loss_probability:{:.4f},\t loss_kld:{:.4f},\t \
            kld_w:{:.4f},\t z_clip:{:.4f} ".format(
            epoch, optimizer.param_groups[0]['lr'], loss_dict['loss_goal'], loss_dict['loss_traj'], loss_dict['loss_probability'],
            loss_dict['loss_kld'], loss_dict['kld_weight'], loss_dict['z_logit_clip'])
...
def post_process(cfg, X_global, y_global, pred_traj, pred_goal=None, dist_traj=None, dist_goal=None):
    ...
    if dim == 4:
        ...
        if hasattr(dist_traj, 'mus') and cfg.DATASET.NORMALIZE != 'none':
            ...
            if cfg.DATASET.NORMALIZE == 'zero-one':     #zero-one
