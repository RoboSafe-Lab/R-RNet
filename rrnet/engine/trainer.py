        ...
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            if cfg.MODEL.LATENT_DIST == 'categorical':  # gaussian
                loss = loss_dict['loss_goal'] + \
                       loss_dict['loss_traj'] + \
                       loss_dict['loss_probability'] + \
                       loss_dict['loss_attention'] + \
            ...
            if cfg.VISUALIZE and iters % max(int(len(dataloader) / 5), 1) == 0:
                ret = post_process(cfg, X_global, y_global, pred_traj, pred_goal=pred_goal,
                                   dist_goal=dist_goal)
...
def do_val(cfg, epoch, model, dataloader, device, logger=None):
    ...
    with torch.set_grad_enabled(False):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            if cfg.MODEL.ATTENTION:
                loss += loss_dict['loss_attention']
                loss_attention_val += loss_dict['loss_attention'].item()
            loss_goal_val += loss_dict['loss_goal'].item()
            loss_traj_val += loss_dict['loss_traj'].item()
            loss_KLD_val += loss_dict['loss_kld'].item()
            loss_prob_val += loss_dict['loss_probability'].item()

    loss_goal_val /= (iters + 1)

    loss_traj_val /= (iters + 1)
    loss_KLD_val /= (iters + 1)
    loss_prob_val /= (iters + 1)
    loss_val = loss_goal_val + loss_traj_val + loss_KLD_val + loss_prob_val
    if cfg.MODEL.ATTENTION:

        loss_attention_val /= (iters + 1)
        loss_val += loss_attention_val
        info = "loss_val:{:.4f}, \
                loss_attention_val:{:.4f}, \
                loss_goal_val:{:.4f}, \
                loss_traj_val:{:.4f}, \
                loss_prob_val:{:.4f}, \
                loss_kld_val:{:.4f}".format(loss_val, loss_attention_val, loss_goal_val, loss_traj_val, loss_prob_val,
                                            loss_KLD_val)

        if hasattr(logger, 'log_values'):
            logger.info(info)
            logger.log_values({'loss_val': loss_val,
                               'loss_revised_val': loss_attention_val,
                               'loss_goal_val': loss_goal_val,
                               'loss_traj_val': loss_traj_val,
                               'loss_prob_val': loss_prob_val,
...
def inference_kde_nll(cfg, epoch, model, dataloader, device, logger=None):
    ...
    Latent_sample_num_test = model.Latent_sample_num_test
    model.Latent_sample_num_test = 2000
    num_samples = model.K
    model.K = 2000
    with torch.set_grad_enabled(False):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
    ...
    model.K = num_samples
    model.Latent_sample_num_test = Latent_sample_num_test
    return KDE_NLL
