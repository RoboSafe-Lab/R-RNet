...
from rrnet.modeling import make_model

from rrnet.engine import build_engine
from rrnet.utils.scheduler import ParamScheduler, sigmoid_anneal
from rrnet.utils.logger import Logger
import logging
...
def build_optimizer(cfg, model):
    ...
    #optimizer = optim.Adam(all_params, lr=cfg.SOLVER.LR)   #,weight_decay=0.01
    return optimizer
...
def main():
    ...
    model = make_model(cfg)  # rrnet/modeling/rrnet.py
    ...
    if hasattr(logger, 'run_id'):
        ...
        run_id = 'no_wandb'
    save_checkpoint_dir = os.path.join(cfg.OUT_DIR, 'weights')
